...
# write_rds(data_ready, 'team3_ecrf_wide.rds')
...
Нулевая гипотеза: Разница средних между группами "Куркумин НЕО" и "Метотрексат" меньше или равна порогу не меньшей эффективности. (то есть "Куркумин НЕО" имеет такой же или даже более выраженный эффект)

Альтернативная гипотеза: Разница средних между группами "Куркумин НЕО" и "Метотрексат" больше порога не меньшей эффективности. (то есть "Куркумин НЕО" имеет клинически значимо меньший эффект, чем "Метотрексат")
...
ggplot(data_ready, aes(x = ARM, y = delta_DAS28)) +
  ...
  labs(
    title = 'Сравнение средней разницы оценки по шкале DAS28\n по граппам терапии',
    subtitle = 'Сравнение по терапии: Т-тест',
    x = NULL,
    y = "Разница оценки по шкале DAS28\n между 12 и 0 визитом"
  )+
  scale_fill_manual(
    values = ARMvalues
  )+
  scale_x_discrete(labels = ARMlabels)+
  theme(
...
Альтернативная гипотеза: Разница средних между группами “Куркумин НЕО” и метотрексата больше 0. Т.е. “Куркумин НЕО” имеет статистически значимо больший эффект в сравнении с “Метотрексат”.
...
data_ready %>% 
  ...
  labs(
    title = 'Сравнение средней разницы оценки по шкале VAS\n по граппам терапии',
    subtitle = 'Сравнение по терапии: Т-тест',
    x = NULL,
    y = "Разница оценки по шкале VAS\n между 12 и 7 визитом"
  )+
  scale_fill_manual(
    values = ARMvalues
  )+
...
#### Измерение доли пациентов, достигших полного ответа по шкале DAS28 (DAS28 < 2,6)

В результате анализа количества пациентов, достигнувших полного ответа на лечение (DAS28 < 2.6) через 12 месяцев, не выявили статистически значимых различий между группами терапии. В группе “Куркумин НЕО” ни один пациент не достиг полного ответа на лечение, в группе “Метотрексат” полный ответ на лечение достигли 5 из 13 пациентов (38,5%). 

Согласно полученному критерию Хи-квадрат для проверки независимости, терапия и достижение полного ответа на лечение не имеют статистически значимую взаимосвязь по уровню значимости 0.05 (p = 0.0878, X-квадрат = 2.91).

```{r warning=FALSE}
tableV12 <- table(data_ready$ARM, data_ready$DAS28min_V12)

chiDAS28 <- chisq.test(tableV12)
```

**Отношение шансов** между группами "Куркумин НЕО" и “Метотрексат” равно нулю, так как ни один из пациентов в группе "Куркумин НЕО" не достиг полного ответа.

```{r}
data_ready %>%
  mutate(
    ARM = case_when(
      ARM == 'Methotrexate' ~ "Метотрексат",
      ARM == 'CurcuminNEO' ~ "Куркумин НЕО",
      TRUE ~ ARM 
    )
  ) %>% 
  group_by(ARM) %>% 
  summarise(
    "Не достигли полного ответа" = sum(DAS28min_V12 == '>=2.6'),
    "Достигли полного ответа" = sum(DAS28min_V12 == '<2.6'),
    "Всего" = n()
  )  %>% 
  bind_rows(
    summarise(., across(where(is.numeric), sum), ARM = "Всего")) %>% 
  flextable() 
```

#### Измерение доли пациентов, достигших приемлемого для себя состояния симптомов (PASS) 

В результате анализа количества пациентов, достигнувших приемлемого для себя состояния симптомов (PASS = 0), не выявили статистически значимых различий между группами терапии. Согласно полученному критерию Хи-квадрат для проверки независимости, терапия и достижение приемлемого для себя состояния симптомов не имеют статистически значимой взаимосвязи по уровню значимости 0.05 (p = 1, X-квадрат = 0).
...
data_ready %>%
  ...
  group_by(ARM) %>% 
  summarise(
    "Отсутсвие боли" = sum(V12_PASS_PASS == '0'),
    "Боль сохранилась" = sum(V12_PASS_PASS == '1'),
    "Всего" = n()
  )  %>% 
  bind_rows(
    summarise(., across(where(is.numeric), sum), ARM = "Всего")) %>% 
...
```{r warning=FALSE}
tablePASS_V12 <- table(data_ready$ARM, data_ready$V12_PASS_PASS)

chiPASS <-chisq.test(tablePASS_V12)
```

Отношение шансов достижения приемлемого для себя состояния симптомов группы терапии “Куркумин НЕО”, к группе терпии “Метотрексат” (OR = 1.16, 95% ДИ [0.21; 6.52], р-значение = 0.85) указывает на отсутсиве статистически значимых различий. 

```{r warning=FALSE}
orPASS <- oddsratio(tablePASS_V12)
```
